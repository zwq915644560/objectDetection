...
brightness_list =[-60, -30, 0, 15, 30]
angles=range(0, 360, 45)# [0,45,90,135,180,225,270,315],8
offs=[-60, -30, 0, 30, 60]# [-40,-20,0,20,40],5
...
for file_ in fileNames:
    ...
    for brightness in brightness_list:
        tmp1=contrast_brightness(srcImg, 1, brightness)
        #cv2.imshow('brightness', dstImg)
        # 翻转
        for flag in range(2):
            if flag==0:
                tmp2=tmp1
            else:
                tmp2=flip(tmp1, 'left_right')
            # 旋转
            for angle in angles:
                tmp3=rotate(tmp2, angle)
                # 平移
                for off in offs:
                    tmp4=offset(tmp3, off, 0)
                    ...
                    for flag1 in range(2):
                        if flag1==0:
                            ...
                            rand=np.empty(12)
                            for i in range(12):
                                rand[i]=np.random.choice(rands)
                            pts1 = np.float32([[rand[0],rand[1]],[200+rand[2],rand[3]],[rand[4],200+rand[5]]])
                            pts2 = np.float32([[rand[6],10+rand[7]],[200+rand[8],50+rand[9]],[10+rand[10],200+rand[11]]])
                            tmp5=affine(tmp4, pts1, pts2)
                        c+=1
                        if c==NUM_PER_CLASS+1:
                            break
                        pn=newDir+fileName+str(c)+'.jpeg'
                        cv2.imwrite(pn, tmp5, [int(cv2.IMWRITE_JPEG_QUALITY), 95])
                        img, loc=objectRecognition(pn)
                        #cv2.imwrite(pn, img, [int(cv2.IMWRITE_JPEG_QUALITY), 95])
                        #fd=open(path+fileName+'.txt', 'a')
                        fd.write(str(c)+' '
								+str(loc[0][0])+' '
								+str(loc[0][1])+' '
